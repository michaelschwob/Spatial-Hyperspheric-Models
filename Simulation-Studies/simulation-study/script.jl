...
include("mcmc_nonspatial.jl")
include("mcmc_spatial.jl")
nMCMC = 100000
nBurn = Int(0.2*nMCMC)
nKeep = 1000
nSamp = 10000
...
X = [ones(n) X]
Xo = copy(X) # original X prior to standardizing
[X[:, i] = dirStandard(X[:, i]) for i in 2:p]

## regression coefficients
B = [2.0 0 0; 2.5 2.3 0.5; 2.2 1.1 3.3].*0.5
...
eta_vec_aug = rand(MvNormal(zeros(n_aug*d), Hermitian(Sig_eta_aug)), 1)
eta_mat_aug = reshape(eta_vec_aug, d, n_aug)
eta_vec = eta_vec_aug[1:(n*d)]
eta_mat = reshape(eta_vec, d, n)

## uncertainty propagation term
z = rand(Uniform(0, 1), n)
q = size(z, 2)
alphas = rand(Normal(0, 0.5), q)
A = Matrix(kronecker(ones(d), alphas))
...
## compute V
gammas = [rand(Truncated(Normal(0, 0.5), 0, Inf), j+1) for j in 1:(d-2)]
V = zeros(d, d, n)
Vinv = zeros(d, d, n)
xi = zeros(d, d, n)
...
## spatial field #1
obs1 = heatmap(x, y, reshape(etaGrid_mat[1, :], ngridpoints, ngridpoints)',
    color = :viridis, title = "True η Grid w/ Observations", label = "", widen = false,
    clim = extrema(etaGrid_mat[1, :]), alpha = 0.75, legend = false, colorbar = :left,
...
## spatial field #2
obs2 = heatmap(x, y, reshape(etaGrid_mat[2, :], ngridpoints, ngridpoints)',
    color = :viridis, title = "", label = "", widen = false,
    clim = extrema(etaGrid_mat[2, :]), alpha = 0.75, legend = false, colorbar = :left,
    xlims = extrema(x), ylims = extrema(y), colorbar_title = "η₂"
)
...
## spatial field #3
obs3 = heatmap(x, y, reshape(etaGrid_mat[3, :], ngridpoints, ngridpoints)',
    color = :viridis, title = "", label = "", widen = false,
    clim = extrema(etaGrid_mat[3, :]), alpha = 0.75, legend = false, colorbar = :left,
    xlims = extrema(x), ylims = extrema(y), colorbar_title = "η₃"
)
...
combined_obs = plot(obs1, obs2, obs3, layout = (3, 1), size = (600, 1000))
savefig(combined_obs, "truth_obs.pdf")
...
[X_grid[:, i] = dirStandard_grid(X_grid[:, i], Xo[:, i]) for i in 2:p]
XG = copy(X_grid)
...
###
### Fit Non-Spatial ESAG⁺ MCMC
###

println("Non-spatial ESAG+ MCMC:")
...
## C (coded as Am) traces ; should be constant
plots = []
for i in 1:d
    for j in 1:d
...
## phi indices ; should be constant
plots = []
for i in 1:d
...
mu_pred = [link.(B_post*X_grid[i, :]) for i in 1:n_grid]
...
pred1 = heatmap(x, y, reshape(mu_pred_mat[1, :], ngridpoints, ngridpoints)',
    color = :viridis, title = "Estimated NMD (Standard)", titlefontsize = 25, label = "", widen = false,
...
println("Predicting NMD (Standard):")
...
@save "MCMCoutput.jld2" out etaPred_mat logS logS_mean
...
out_trunc = mcmc_spatial(Y, X, z, spatial_idx, nMCMC, nBurn, nSamp, 1, 5, true, R_list, phiIdx_vec)
...
plot(plots..., layout = (d, p), size = (900, 600), left_margin = 10mm, legend = false)
savefig("betaTraces_ESAG_spatial.pdf")
...
plot(plots..., layout = (d, 5), size = (900, 600), left_margin = 10mm, legend = false)
savefig("etaTraces_ESAG_spatial.pdf")
...
plot(plots..., layout = (d, d), size = (900, 600), left_margin = 10mm, legend = false)
savefig("AmTraces_ESAG_spatial.pdf")
...
plot(plots..., layout = (d, 1), size = (900, 600), left_margin = 10mm, legend = false)
savefig("phiTraces_ESAG_spatial.pdf")
...
savefig(pred_obs, "pred_obs_spatial.pdf")

###
### Truth and Pred w/ Observations
###

tog_obs = plot(obs1, pred1_trunc, obs2, pred2_trunc, obs3, pred3_trunc, layout = (3, 2), size = (1100, 1000), left_margin = 25mm, right_margin = 15mm)
savefig(tog_obs, "tog_obs_spatial.pdf")
...
nmd_obs = plot(nmd1, nmd2, nmd3, layout = (3, 1), size = (600, 1000))
savefig(nmd_obs, "nmd_obs_spatial.pdf")
...
savefig(mean_dir, "tog_norm_mean_dir_spatial.pdf")
...
###
### Compute normalized mean directions (spatial)
###
...
@save "MCMCoutput_spatial.jld2" out_trunc etaPred_mat logS_trunc logS_mean_trunc
